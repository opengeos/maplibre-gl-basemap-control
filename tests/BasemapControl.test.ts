...
function createMockMap() {
  ...
  const map = {
    ...
    on: vi.fn((type: string, fn: (event?: unknown) => void) =>
      addListener(type, fn, false),
    ),
    once: vi.fn((type: string, fn: (event?: unknown) => void) =>
      addListener(type, fn, true),
    ),
    ...
    addLayer: vi.fn(
      (layer: { id: string; type: string }, beforeId?: string) => {
...
describe("BasemapControl", () => {
  ...
  it("creates the collapsed button and renders basemap results", () => {
    ...
    expect(
      screen
        .getByLabelText("Search basemaps")
        .closest(".basemap-control-search-row"),
    ).toBe(
    ...
    expect(
      document
        .querySelector(".basemap-control-panel")
  ...
  it("filters results from the search input", () => {
    ...
    controlCorner.appendChild(control.onAdd(map as never));
    const searchInput =
      screen.getByLabelText<HTMLInputElement>("Search basemaps");
    searchInput.focus();
  ...
  it("preserves the results scroll position when selecting a basemap", async () => {
    ...
    expect(
      document.querySelector<HTMLElement>(".basemap-control-results")
        ?.scrollTop,
    ).toBe(42);
  });
  ...
  it("uses the before_id input when adding raster basemaps", async () => {
    ...
    controlCorner.appendChild(control.onAdd(map as never));
    fireEvent.input(screen.getByLabelText("before_id"), {
  ...
  it("shows provider settings when keyed provider basemaps are available", () => {
    ...
    expect(
      screen.getByLabelText<HTMLInputElement>("MapTiler API key").value,
    ).toBe("");
    expect(
      screen.getByLabelText<HTMLInputElement>("Mapbox access token").value,
    ).toBe("");
    expect(
      screen.getByLabelText<HTMLInputElement>("Mapbox access token")
        .autocomplete,
    ).toBe("new-password");
    expect(
      screen.getByLabelText<HTMLInputElement>("Amazon API key").value,
    ).toBe("");
    expect(screen.getByLabelText<HTMLInputElement>("AWS region").value).toBe(
      "us-east-1",
    );
  });
  ...
  it("makes a missing-credential error actionable with a help link and reveals the inputs", async () => {
    ...
    const link = status?.querySelector<HTMLAnchorElement>(
    ...
    const inlineFields = status?.querySelector(
      ".basemap-control-status-fields",
    );
    expect(inlineFields).toBeTruthy();
    expect(
      inlineFields?.querySelector('[aria-label="Amazon API key"]'),
    ).toBeTruthy();
    expect(
      inlineFields?.querySelector('[aria-label="AWS region"]'),
    ).toBeTruthy();
  ...
  it("requires an API key before applying Stadia basemaps", async () => {
    ...
    await expect(
      control.setBasemap("stadia-stamen-watercolor"),
    ).rejects.toThrow(
  ...
  it("stacks raster basemaps when allowMultiple is enabled", async () => {
    ...
    expect(control.getActiveBasemaps().map((basemap) => basemap.id)).toEqual([
      "one",
      "two",
    ]);
    expect(control.getActiveBasemap()?.id).toBe("two");
  ...
  it("clears a stale credential error when the user searches for an alternative", async () => {
    ...
    await expect(control.setBasemap("amazon-standard")).rejects.toThrow();
    expect(
      document.querySelector(".basemap-control-status.is-error"),
    ).toBeTruthy();
  ...
  it("retries the failed basemap from the inline credential field on Enter", async () => {
    ...
    expect(
      document.querySelector(".basemap-control-status-fields"),
    ).toBeTruthy();
  ...
  it("ignores non-auth resource errors on a style that loaded", async () => {
    ...
    map.fire("error", {
      error: { url: "https://example.com/tiles/1/2/3.png", status: 404 },
    });
  ...
  it("does not roll back a keyless style basemap on a transient tile error", async () => {
    ...
    map.fire("error", {
      error: { url: "https://example.com/tiles/9/9/9.png", status: 403 },
    });
  ...
  it("switches between replace and add modes via the panel toggle", async () => {
    ...
    const toggle = screen.getByLabelText<HTMLInputElement>(
      "Add basemaps instead of replacing",
    );
  ...
  it("hides the multiple toggle when showMultipleToggle is false", () => {
    ...
    controlCorner.appendChild(control.onAdd(map as never));
    expect(
      screen.queryByLabelText("Add basemaps instead of replacing"),
    ).toBeNull();
  });
  ...
  it("renders resize handles by default and omits them when disabled", () => {
    ...
    expect(
      document.querySelectorAll(".basemap-control-resize-handle"),
    ).toHaveLength(2);
    resizable.onRemove();
...
function lastSourceFor(
  map: ReturnType<typeof createMockMap>["map"],
  basemapId: string,
) {
  const sourceId = `maplibre-basemap-control-source-${basemapId}`;
  const call = [...map.addSource.mock.calls]
    .reverse()
    .find((args) => args[0] === sourceId);
  return call?.[1] as { tiles?: string[]; url?: string } | undefined;
}

function lastLayerFor(
  map: ReturnType<typeof createMockMap>["map"],
  basemapId: string,
) {
  const call = [...map.addLayer.mock.calls]
    .reverse()
    .find((args) => args[0]?.id === basemapId);
  return call?.[0] as
    | {
        id: string;
        type: string;
...
describe("BasemapControl traffic overlays", () => {
  ...
  it("reports a missing TomTom key and skips adding the layer", async () => {
    ...
    await expect(
      control.addBasemap("tomtom-traffic-flow-relative"),
    ).rejects.toThrow(/TomTom API key/);
  ...
  it("reports a missing Mapbox token for the traffic overlay", async () => {
    ...
    await expect(control.addBasemap("mapbox-traffic")).rejects.toThrow(
      /Mapbox access token/,
    );
  ...
  it("creates a Google tile session and substitutes the token, caching it across adds", async () => {
    ...
    expect(url).toBe(
      "https://tile.googleapis.com/v1/createSession?key=g-secret",
    );
  ...
  it("reports a Google session failure with the API error message", async () => {
    ...
    globalThis.fetch = vi.fn(async () => ({
      ...
      text: async () =>
        JSON.stringify({
          error: { message: "Map Tiles API has not been used" },
        }),
    })) as never;
    ...
    await expect(control.addBasemap("google-traffic")).rejects.toThrow(
      /Map Tiles API/,
    );
  ...
  it("offers an optional key input after a base Google basemap falls back to public tiles", async () => {
    ...
    expect(
      document.querySelector(".basemap-control-optional-key"),
    ).toBeTruthy();
    expect(
      screen.getByLabelText<HTMLInputElement>("Google Maps API key"),
    ).toBeTruthy();
  });
